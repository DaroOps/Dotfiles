...
export const getClientsWithEmployeeInfo = async (clients) => {
...
export const getClientsAndEmployeesNames = async () => {
    ...
    let result = await getClientsWithEmployeeInfo(clients);
...
export const getClientsWhoMadePaymentsAndTheyCity = async () => {
    ...
    const filteredClients = await Promise.all(
      clientsWithEmployeeInfo.filter(async (client) => {
        const payments = await getPaymentByClientCode(client.client_code);
        return payments.length > 0;
      })
    );
  
    return await getClientsWithEmployeeAndOfficeInfo(filteredClients);
  };
