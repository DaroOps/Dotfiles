...
export const clientsWhoReceivedTheirRequestLate = async () => {
    try {
        const lateRequests = await getAllLateRequests(); // Assuming this function returns a list of late requests
        console.log("late", lateRequests);

        // Extract unique client codes from the late requests
        const clientCodes = [...new Set(lateRequests.map(({ clientCode }) => clientCode))];
        console.log("received request late", clientCodes);

        // Fetch client names for each unique client code
        ...
        console.error("Error fetching clients who received their requests late:", error);
