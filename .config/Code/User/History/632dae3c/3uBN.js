...
export const getRequestDetailsByProductCode = async (code) => {
  const res = await fetch(`${end}?product_code=${code}`);
  const details = await res.json();
  return details;
};

export const getRequestDetailsByRequest = async (code) => {
  const res = await fetch(`${end}?code_request=${code}`);
  const details = await res.json();
  return details;
};
...
export const lisOfProductRangesPurchasedByClient = async () => {
  const res = await fetch("${end}?_expand=product&_expand=request&_embed=request.client");
...
const getSalesRepsWithFruitPurchases = async (offices) => {
    ...
    const fruitRequestDetails = await (await fetch("${end}?join=inner&product_gama=Frutales")).json();
