...
export const getCEOInformation = async () => {
    ...
    if (ceoEmployee) {
        return {
            puesto: ceoEmployee.puesto, // Ajusta esto según la estructura real de tus datos
            nombre: ceoEmployee.nombre,
            apellidos: `${ceoEmployee.apellidos}`,
            email: ceoEmployee.email.match(/(?<=\[)[^\[\]]+@[^@\[\]]+(?=\])/)[0]
        };
    } else {
        return "No se encontró al CEO.";
    }
};
