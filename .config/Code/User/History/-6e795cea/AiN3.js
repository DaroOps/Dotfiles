...
export const getEmployeesWithBoss = async () => {
    const employees = await getEmployeesByCode();
...
export const getEmployeesWithBosses = async () => {
    ...
    for (const employee of employees) {
        const employeeData = await getEmployeeData(employee);
        const bossOfTheBoss = employeeData.code_boss === null ? 'No tiene' : await getNameByEmployeeCode(await getEmployByCode2(employeeData.code_boss).code_boss);
        bosses.push({
