import { getRequestDetailsByProductCode, getGamaByName  } from `../helpers/helpers`;
...
export const getProductByCode = async (code) => {
 const res = await fetch(`http://localhost:5506/products?code_product=${code}`);
...
export const getProductsWithGammaOrnamentales = async () => {
 const res = await fetch("http://localhost:5506/products?gama=Ornamentales&stock_gt=100");
 const data = await res.json();
...
export const getProductsWithoutRequest = async () => {
 const res = await fetch("http://localhost:5506/products");
